...
namespace myns
{
	...
	//キューを表示
	template<class T> constexpr std::ostream& operator<<(std::ostream& os, const std::queue<T>& v) noexcept;
	//両端キューを表示
...
template<class T> constexpr std::ostream& myns::operator<<(std::ostream& os, const std::queue<T>& v) noexcept {
	auto q_copy = v;
	if (myns::cout_vector_tatehyoji) {
		while (!q_copy.empty()) {
			os << q_copy.front() << '\n';
...
#include <atcoder/all>
/*
template <class T> constexpr T max(const std::vector<T>& arr) {
	return *max_element(begin(arr), end(arr));
}
template <class T, class Comparator> constexpr T max(const std::vector<T>& arr, Comparator c) {
	return *max_element(begin(arr), end(arr), c);
}
template <class T> constexpr T min(const std::vector<T>& arr) {
	return *min_element(begin(arr), end(arr));
}
template <class T, class Comparator> constexpr T min(const std::vector<T>& arr, Comparator c) {
	return *min_element(begin(arr), end(arr), c);
}
*/

//class Tを追加し、返却値型をTにするとコンパイルできない。
//比較関数(ラムダ)の引数は必ずconst auto&にする。
//比較関数のデフォルト引数をラムダ式で表すと[](const auto& a, const auto& b) {return a < b;}
template <class Container, class Comparator = std::less<>>
constexpr auto max(const Container& arr, Comparator comp = std::less<>{}) {
	return *max_element(begin(arr), end(arr), comp);
}
template <class Container, class Comparator = std::less<>>
constexpr auto min(const Container& arr, Comparator comp = std::less<>{}) {
	return *min_element(begin(arr), end(arr), comp);
}

//my_mapクラス:mapの継承
//mapクラスと違い、operator[]を使用したときキーが存在しない場合のデフォルト値を変更できるようにする。
//at()は std::map::at()をそのまま使用する。
template<class my_key_type, class my_value_type, class my_compare_type = std::less<my_key_type>>
class my_map : public std::map<my_key_type, my_value_type, my_compare_type> {
private:
	using super_class = std::map<my_key_type, my_value_type, my_compare_type>;
	const my_value_type defaultValue;
public:
	my_map(my_value_type defaultValue) : defaultValue(defaultValue) {}
	my_map() : defaultValue(my_value_type()) {}
	my_value_type& operator[](my_key_type&& keyValue) {
		if (!super_class::contains(keyValue)) {
			super_class::operator[](keyValue) = defaultValue;
		}
		return super_class::operator[](keyValue);
	}
	my_value_type& operator[](const my_key_type& keyValue) {
		if (!super_class::contains(keyValue)) {
			super_class::operator[](keyValue) = defaultValue;
		}
		return super_class::operator[](keyValue);
